<?php require 'inc/_global/config.php'; ?>
<?php require 'inc/backend/config.php'; ?>
<?php require 'inc/_global/views/head_start.php'; ?>
<?php require 'inc/_global/views/head_end.php'; ?>
<?php require 'inc/_global/views/page_start.php'; ?>

<!-- Page Content -->
<div class="content">
  <!-- Rounded Themed Blocks -->
  <h2 class="content-heading">Rounded Themed</h2>
  <div class="row">
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header">
          <h3 class="block-title">Primary</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-primary-light">
          <h3 class="block-title">Primary Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-primary-dark">
          <h3 class="block-title">Primary Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-primary-darker">
          <h3 class="block-title">Primary Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-success">
          <h3 class="block-title">Success</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-info">
          <h3 class="block-title">Info</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-warning">
          <h3 class="block-title">Warning</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-danger">
          <h3 class="block-title">Danger</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gray">
          <h3 class="block-title">Gray</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-muted">
          <h3 class="block-title">Gray Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gray-darker">
          <h3 class="block-title">Gray Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-black">
          <h3 class="block-title">Black</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-elegance">
          <h3 class="block-title">Elegance</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-elegance-light">
          <h3 class="block-title">Elegance Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-elegance-dark">
          <h3 class="block-title">Elegance Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-elegance-darker">
          <h3 class="block-title">Elegance Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-pulse">
          <h3 class="block-title">Pulse</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-pulse-light">
          <h3 class="block-title">Pulse Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-pulse-dark">
          <h3 class="block-title">Pulse Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-pulse-darker">
          <h3 class="block-title">Pulse Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-flat">
          <h3 class="block-title">Flat</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-flat-light">
          <h3 class="block-title">Flat Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-flat-dark">
          <h3 class="block-title">Flat Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-flat-darker">
          <h3 class="block-title">Flat Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-corporate">
          <h3 class="block-title">Corporate</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-corporate-light">
          <h3 class="block-title">Corporate Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-corporate-dark">
          <h3 class="block-title">Corporate Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-corporate-darker">
          <h3 class="block-title">Corporate Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-earth">
          <h3 class="block-title">Earth</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-earth-light">
          <h3 class="block-title">Earth Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-earth-dark">
          <h3 class="block-title">Earth Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-earth-darker">
          <h3 class="block-title">Earth Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-aqua">
          <h3 class="block-title">Aqua</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-cherry">
          <h3 class="block-title">Cherry</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-dusk">
          <h3 class="block-title">Dusk</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-emerald">
          <h3 class="block-title">Emerald</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-lake">
          <h3 class="block-title">Lake</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-leaf">
          <h3 class="block-title">Leaf</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-sea">
          <h3 class="block-title">Sea</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed block-rounded">
        <div class="block-header bg-gd-sun">
          <h3 class="block-title">Sun</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
  </div>
  <!-- END Rounded Themed Blocks -->

  <!-- Square Themed Blocks -->
  <h2 class="content-heading">Square Themed</h2>
  <div class="row">
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header">
          <h3 class="block-title">Primary</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-primary-light">
          <h3 class="block-title">Primary Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-primary-dark">
          <h3 class="block-title">Primary Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-primary-darker">
          <h3 class="block-title">Primary Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-success">
          <h3 class="block-title">Success</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-info">
          <h3 class="block-title">Info</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-warning">
          <h3 class="block-title">Warning</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-danger">
          <h3 class="block-title">Danger</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gray">
          <h3 class="block-title">Gray</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-muted">
          <h3 class="block-title">Gray Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gray-darker">
          <h3 class="block-title">Gray Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-black">
          <h3 class="block-title">Black</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-elegance">
          <h3 class="block-title">Elegance</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-elegance-light">
          <h3 class="block-title">Elegance Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-elegance-dark">
          <h3 class="block-title">Elegance Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-elegance-darker">
          <h3 class="block-title">Elegance Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-pulse">
          <h3 class="block-title">Pulse</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-pulse-light">
          <h3 class="block-title">Pulse Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-pulse-dark">
          <h3 class="block-title">Pulse Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-pulse-darker">
          <h3 class="block-title">Pulse Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-flat">
          <h3 class="block-title">Flat</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-flat-light">
          <h3 class="block-title">Flat Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-flat-dark">
          <h3 class="block-title">Flat Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-flat-darker">
          <h3 class="block-title">Flat Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-corporate">
          <h3 class="block-title">Corporate</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-corporate-light">
          <h3 class="block-title">Corporate Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-corporate-dark">
          <h3 class="block-title">Corporate Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-corporate-darker">
          <h3 class="block-title">Corporate Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-earth">
          <h3 class="block-title">Earth</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-earth-light">
          <h3 class="block-title">Earth Light</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-earth-dark">
          <h3 class="block-title">Earth Dark</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-earth-darker">
          <h3 class="block-title">Earth Darker</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-aqua">
          <h3 class="block-title">Aqua</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-cherry">
          <h3 class="block-title">Cherry</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-dusk">
          <h3 class="block-title">Dusk</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-emerald">
          <h3 class="block-title">Emerald</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-lake">
          <h3 class="block-title">Lake</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-leaf">
          <h3 class="block-title">Leaf</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-sea">
          <h3 class="block-title">Sea</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="block block-themed">
        <div class="block-header bg-gd-sun">
          <h3 class="block-title">Sun</h3>
          <div class="block-options">
            <button type="button" class="btn-block-option">
              <i class="si si-wrench"></i>
            </button>
          </div>
        </div>
        <div class="block-content">
          <p>Block’s content..</p>
        </div>
      </div>
    </div>
  </div>
  <!-- END Square Themed Blocks -->
</div>
<!-- END Page Content -->

<?php require 'inc/_global/views/page_end.php'; ?>
<?php require 'inc/_global/views/footer_start.php'; ?>
<?php require 'inc/_global/views/footer_end.php'; ?>